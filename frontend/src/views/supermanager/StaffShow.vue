      ...
      <div class="farm-card farm-card-static">
        ...
        <div class="staff-show-content">
          <table class="farm-table">
            ...
            <tbody>
              <tr v-for="(staff, index) in staffList1" :key="index" class="farm-table-row">
    ...
    <div class="position-modal" v-if="showPositionModal">
      ...
      <div class="position-modal-content">
        <div class="position-modal-header">
          <h3 class="position-modal-title">{{ staff && staff.position ? "调任" : "任职" }}</h3>
          <button class="position-modal-close" @click="closePositionModal">&times;</button>
        </div>
        <div class="position-modal-body">
          <div class="form-group">
            <label class="farm-form-label">选择职位</label>
            <select class="farm-form-control" v-model="position">
              <option v-for="(pos, index) in positionList" :key="index" :value="pos">
                {{ pos.name }}
              </option>
            </select>
          </div>
    ...
    <div class="confirm-overlay" v-if="showConfirmDialog" @click="cancelConfirm">
      <div class="confirm-dialog" @click.stop>
        ...
        <div class="confirm-buttons">
          ...
          <button class="farm-btn farm-btn-danger" @click="executeDelete">确认</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    takeOffice() {
      if (this.staff.position != null && this.staff.position.name == this.position.name) {
